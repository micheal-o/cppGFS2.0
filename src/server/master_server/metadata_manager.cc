...
MetadataManager::GetFileChunkMetadata(const std::string& chunk_handle) {
  ...
  protos::FileChunkMetadata metadata = try_get_chunk_data.first;
  auto locations = chunk_server_manager_.GetChunkLocations(chunk_handle);
  for (auto location: locations) {
    *metadata.add_locations() = location;
  }
  return metadata;
}
...
google::protobuf::util::Status MetadataManager::SetPrimaryChunkServerLocation(
    const std::string& chunk_handle,
